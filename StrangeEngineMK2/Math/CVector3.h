...
public:
    ...
    __declspec(dllexport) CVector3& operator*= (const float s);
};
...
__declspec(dllexport) CVector3 operator+ (const CVector3& v, const CVector3& w);

// Vector-vector subtraction
__declspec(dllexport) CVector3 operator- (const CVector3& v, const CVector3& w);

// Vector-scalar multiplication
__declspec(dllexport) CVector3 operator* (const CVector3& v, float s);
__declspec(dllexport) CVector3 operator* (float s, const CVector3& v);
...
__declspec(dllexport)  float Dot(const CVector3& v1, const CVector3& v2);

// Cross product of two given vectors (order is important) - non-member version
__declspec(dllexport)  CVector3 Cross(const CVector3& v1, const CVector3& v2);

// Return unit length vector in the same direction as given one
__declspec(dllexport)  CVector3 Normalise(const CVector3& v);

// Returns length of a vector
__declspec(dllexport) float Length(const CVector3& v);
